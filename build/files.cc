...
static char *lookupReadinFile(const char *path)
{
    char *filename = NULL;
    char *searchPath;
    ARGV_t searchDirs;
    ...
    searchPath = rpmExpand("%{__readinspath}", NULL);
    searchDirs = argvSplitString(searchPath, ":", ARGV_SKIPEMPTY);

    for (ARGV_t d = searchDirs; *d; d++) {
